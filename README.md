# 8 FOSS for Content-Creators
There are anumber of apps that make life of a contetn-creator easy. These are some software that are free and have exceptional features to be a contendor for professional work.

## GIMP
Supported on : Linux, Mac, Windows

Application: Photo editing, posters, social media material

It is a photo editing software that can be a replacement for Photoshop.

## Inkscape
Supported on: Linux, Mac, Windows
Application: Logo, illustrations

It can be a great alternatibe for Adobe graphics suite.

## Krita
Supported on: Linux, Mac, Windows
Application: Digital art(drawing on tablet)

It is fit for users who like to draw on tablets.

## Blender
Supported on: Linux, Microsoft Windows, Mac
Applications: 3D animation

It is a software for video editing and 3D magic.

## Kdenlive
Supported on: Linux, Windows, Mac
Applications: Video editing with multi-track editing

It is best suited for creating YouTube tutorials and vlogs.

## Audacity
Supported on: Windows, Linux, Mac
Applications: Audio editing

It is an audio editing tool, best suited for podcasts and cleanup.

## OBS Studio
Supported on: Linux, Mac, Windows
Applications: Streaming and screen recording

It is a screen recording screen and live-streaming.

## Libre Writer
Supported on: Linux, Mac, Microsoft Windows
Applications: Document writing, doc editing 

It is a document writer and editor that supported various formats including Microsoft Word (upto 2010 edition). 